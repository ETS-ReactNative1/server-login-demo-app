        ...
        if (result.rows[0]) {
          getHashPassword(password1)
            ...
            .catch((e) => {
              ...
              res
                .status(500)
                .send(JSON.stringify({ msg: "Internal server error" }));
            });
        } else {
          res
            .status(400)
            .send(JSON.stringify({ msg: "Token does not exist or expired." }));
        }
      })
      .catch((e) => {
  ...
  checkEmailUser(email)
    .then((result) => {
      if (result.rows[0]) {
        ...
        res
          .status(400)
          .send(JSON.stringify({ msg: "Your email does not exist bro." }));
      }
    })
    .catch((e) => {
      console.log(e);
